...
const VoiceAssistantSettings: React.FC<VoiceAssistantSettingsProps> = ({
  ...
  useEffect(() => {
    ...
    const providedKey = import.meta.env.VITE_HUGGINGFACE_TOKEN || "";
    ...
    <Sheet>
      ...
      <SheetContent className="w-[350px] sm:w-[450px]">
            ...
            <p className="text-sm text-muted-foreground">
              ...
              {!apiKey && (
                <span className="block mt-1 text-amber-500">
                  Currently using demo mode with mock responses. Add your API
                  key for real AI responses.
                </span>
              )}
              {apiKey && (
                <span className="block mt-1 text-green-500">
                  API key is set! You're now using the Hugging Face API for real
                  AI responses.
                </span>
              )}
            </p>
        ...
        <SheetFooter className="mt-4 text-xs text-muted-foreground">
          Note: Your API key is stored only in your browser's local storage and
